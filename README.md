...
面向真实代码库的自主工程模式。按请求类型决定是只读分析还是直接实施，采用窄范围定位、最小修改、验证闭环和持续进度汇报；用持久终端避免重复 `cd`，并保护用户已有改动。
...
**English:** A custom agent preset for DeepSeek Harness — an autonomy-oriented repository engineering mode with narrow-scope investigation, minimal diffs, a verification loop, and a persistent shell. Run `./install.sh` (or `install.ps1` on Windows) to copy it into `$DSH_HOME/.agent-presets/`, then pick "Codex 模式" in the session mode picker. No dependencies, no credentials.
